"""
Train the SASRec-style CTR model (Task 2).

Output:
    outputs/checkpoints/task2_best.pt  — best-AUC checkpoint
"""
...
def stratified_subsample(dataset: CTRDataset, n: int, rng: np.random.Generator) -> Subset:
    """Return a Subset of size n with roughly equal positive/negative ratio."""
    ...
    # top up positives if negatives were scarce (unlikely but safe)
...
def train():
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        if epoch == UNFREEZE_EPOCH + 1:
            model.unfreeze_item_emb()
            print(f"epoch {epoch}: item embeddings unfrozen")
        ...
        eta_str = ""
        if epoch == 1:
            avg_epoch_s = epoch_times[0]
            remaining_epochs = MAX_EPOCHS - 1
            eta_s = avg_epoch_s * remaining_epochs
            eta_str = f"  (est. remaining: {eta_s/60:.1f} min for {remaining_epochs} more epochs)"
